 ...
 * shape is derived from on-chain reads. Name and version come from the
 * token's ERC-5267 `eip712Domain()` (falling back to a default), and
 * capabilities come from Strimz's TokenWhitelist contract.
 *
...
export const tokenMetadataSchema = z.object({
   ...
   * A domain mismatch produces a different digest, so the contract's
 ...
 * owner monotonic counter, and every successful permit consumes one,
