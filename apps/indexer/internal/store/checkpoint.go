...
func (s *Store) LoadCheckpoint(ctx context.Context, env, addr string) (*Checkpoint, error) {
	...
	err := s.pool.QueryRow(ctx, `
		SELECT "contractAddress", environment::text, "lastProcessedBlock", "lastProcessedLogIndex"
		FROM "IndexerCursor"
		WHERE "contractAddress" = $1 AND environment::text = $2
	`, addr, env).Scan(&cp.ContractAddress, &cp.Environment, &cp.LastProcessedBlock, &cp.LastProcessedLogIndex)
	if err != nil {
...
// projected block range. Atomic with the projection writes via `withinTx`.
func (s *Store) SaveCheckpoint(ctx context.Context, cp *Checkpoint) error {
	_, err := s.pool.Exec(ctx, `
		...
		VALUES ($1, $2::"ArcEnvironment", $3, $4, NOW())
		ON CONFLICT ("contractAddress") DO UPDATE
		  SET environment = EXCLUDED.environment,
		      ...
		      "updatedAt" = NOW()
	`, cp.ContractAddress, cp.Environment, cp.LastProcessedBlock, cp.LastProcessedLogIndex)
	if err != nil {
