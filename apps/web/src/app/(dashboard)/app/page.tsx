...
const KPIS = [
  {
    ...
    icon: Wallet,
    href: '/app/analytics',
    subtle: 'No active subscriptions yet',
  },
  {
...
const STEPS = [
  ...
  },
  {
    ...
    d: 'Test mode is free. Live mode unlocks after KYB + 2FA.',
    href: '/app/api-keys',
  },
  {
    n: 3,
    t: 'Send a test payment',
    d: 'Charge a test session and watch it land in real-time.',
    href: '/app/payment-sessions',
  },
  {
    n: 4,
    t: 'Wire a webhook',
    d: 'Add an HTTPS endpoint and send a test event.',
    href: '/app/webhooks',
  },
] as const
...
export default function DashboardHome() {
  return (
    <>
      <PageHeader
        title="Welcome back"
        description="Here's what's happening across your billing surface."
      />
            ...
            <ul className="mt-4 space-y-3 text-sm">
              {READINESS.map((c) => (
                <li key={c} className="flex items-start gap-2">
                  <div className="bg-muted-foreground/40 mt-2 size-1.5 rounded-full" />
                  {c}
                </li>
              ))}
            </ul>
