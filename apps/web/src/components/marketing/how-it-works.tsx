  ...
  return (
    <section className="relative w-full overflow-hidden bg-[#F9FAFB] px-4 py-20 md:px-6 lg:py-24">
      ...
      >
        ...
        <motion.h2
          variants={fadeUp}
          className="mt-5 font-sora text-[32px] font-[700] leading-[1.06] tracking-[-0.02em] text-[#050020] md:text-[44px] lg:text-[48px]"
        >
          From signature to webhook
          <br className="hidden sm:inline" /> in about <span className="text-[#02C76A]">20 seconds</span>.
        </motion.h2>
        <motion.p
          variants={fadeUp}
      ...
      >
        {STEPS.map((step) => (
          <motion.article
            key={step.label}
            variants={fadeUp}
            className="relative flex flex-col rounded-[16px] border border-[#E5E7EB] bg-white p-6 shadow-sub-card transition-transform hover:-translate-y-0.5"
          >
            <div className="flex items-center justify-between">
              <span
                className={[
                  'flex size-12 items-center justify-center rounded-[12px] shadow-sub-icon',
                  step.accent,
                ].join(' ')}
              >
            ...
            <h3 className="mt-5 font-sora text-[18px] font-[700] leading-[24px] text-[#050020]">
              {step.title}
            </h3>
            <p className="mt-2 font-poppins text-[13px] leading-[22px] text-[#58556A]">
              {step.body}
            </p>
          </motion.article>
        ))}
