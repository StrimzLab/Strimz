...
/**
 * Submits the self-attested onboarding form and stamps
 * `onboardingCompleted: true`. We skip the optimistic update we use
 * in `useUpdateMerchant`: the wizard shell reads `onboardingCompleted`
 * to decide whether to keep the user on `/onboarding`, so we want the
 * flip to be authoritative from the server before we redirect.
 */
export function useOnboardMerchant() {
  ...
  return useMutationWithToast({
    ...
    onSuccess: (merchant) => {
      qc.setQueryData(merchantKeys.me(), merchant)
    },
    onSettled: () => {
      qc.invalidateQueries({ queryKey: merchantKeys.me() })
    },
