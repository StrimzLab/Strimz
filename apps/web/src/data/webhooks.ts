...
export const ALL_EVENT_TYPES = [
  ...
  'subscription.created', 'subscription.charged', 'subscription.charge_failed',
  'subscription.cancelled', 'subscription.lapsed',
  'refund.created', 'refund.completed', 'refund.failed',
  'invoice.created', 'invoice.paid', 'invoice.overdue',
  'agent.action_executed', 'agent.job_proposed',
] as const
...
export const WEBHOOK_ENDPOINTS: WebhookEndpoint[] = range(6).map((i) => ({
  id: id('ep', i + 1),
  url: URLS[i]!,
  description: ['Production receiver', 'Backup receiver', 'Reconciliation worker', 'Slack notifier', 'Audit log'][i % 5]!,
  events: i === 0
    ? [...ALL_EVENT_TYPES]
...
export const WEBHOOK_DELIVERIES: WebhookDelivery[] = range(40).map((i) => {
  ...
  return {
    ...
    attemptCount: status === 'permanently_failed' ? 5 : status === 'retrying' ? Math.floor(rng() * 3) + 2 : 1,
