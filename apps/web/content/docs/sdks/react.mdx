...
The React SDK gives you read-only client hooks and a few components for embedded checkout. It uses **publishable keys** (`pk_test_…` / `pk_live_…`), which are safe to ship in browser bundles.
...
export default function Layout({ children }: { children: React.ReactNode }) {
  return (
    <StrimzProvider config={{ baseUrl: 'https://api.strimz.finance' }}>
      {children}
    </StrimzProvider>
  )
}
