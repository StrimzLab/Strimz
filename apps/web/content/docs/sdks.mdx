...
const refund    = await strimz.refunds.create({ transactionId: 'tx_xyz', amount: '50000000', reason: 'customer_request' })
```
...
The browser SDK uses **publishable** keys (`pk_test_…` / `pk_live_…`) — safe to ship to the client.
