...
The top-level you. Created by Privy on first login. Holds business info (name, sector, country, payout address) and configuration (active capabilities, agent settings). Most resources hang off `merchantId`.
...
You create the refund (which validates against `Transaction.amount` so cumulative refunds can't exceed the original). We return wallet-signing instructions. You broadcast the ERC-20 Transfer. The indexer detects the on-chain transfer and flips the row to `completed`.

## Invoice

A hosted, branded payment page with line items + a backing `PaymentSession`. Useful for B2B billing where the buyer wants to see the breakdown before paying.
...
Every delivery is one BullMQ job; we keep the attempts visible in your dashboard so you can debug a flaky receiver row by row.
