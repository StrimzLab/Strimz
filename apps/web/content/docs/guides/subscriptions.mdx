...
Strimz subscriptions are *pull-based*: the customer signs once, granting your contract permission to charge a fixed amount on a fixed cadence. Strimz's scheduler then charges them every period. No re-prompts, no re-authorizations, no churn from "your bank declined the renewal".
...
// → https://strimz.io/sub/plan_pro_monthly
```
