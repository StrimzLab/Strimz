...
picker for every major one), and signs one message.
...
signs, why the relayer cannot move funds on its own, how the
signature expires.
...
and broadcast the transaction. Arc settled in roughly 13 seconds.
