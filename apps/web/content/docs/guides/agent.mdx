...
You can override the email body with a custom template via
`recoveryNotificationTemplate`. Strimz renders it inside the
standard wrapper (header, unsubscribe footer) so the brand stays
consistent.
...
Hourly, the agent computes a z-score for the last completed clock-hour against the trailing 30-day average for that same hour-of-day. If your revenue dropped more than `nσ` (n derived from your `anomalySensitivity`: `low=3σ`, `medium=2σ`, `high=1σ`), you get an email plus an `AuditLog` entry. The agent only flags drops because revenue surges aren't actionable.
...
The agent needs at least 7 prior data points before it will alert.
Merchants in their first week don't get surprised by false
positives.
...
The agent never signs the deposit itself. Confirming the
recommendation in the dashboard is what enqueues a scheduler job to
sign on your behalf. Fund movement is opt-in per transaction.
...
You don't have to do anything. The customer pays from whatever
chain holds their USDC, and the equivalent USDC lands in your Arc
wallet a few minutes later. The bridge is invisible from your end.
